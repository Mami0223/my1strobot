...
import numpy as np
import json  # JavaScript Object Notation人間にも読みやすく、機械にも読み込ませることができる書き方
import threading
import os
...
planGoseList = json.loads(contentPlanGose)  # jsonの中身を読む
'''
planDataNow
'''
...
def limitCheck(posInput, circlePos, circleLen, outline):  # E
    ...
    if realPosSquare >= shortRadiusSquare and realPosSquare <= longRadiusSquare:
        ...
        if realPosSquare < shortRadiusSquare:
            ...
            cX = circlePos[0]*circlePos[0] + \
                (lineB - circlePos[1]) * \
                (lineB - circlePos[1]) - shortRadiusSquare
            ...
            cX = circlePos[0]*circlePos[0] + \
                (lineB - circlePos[1]) * \
                (lineB - circlePos[1]) - longRadiusSquare
...
def planeLinkageReverse(linkageLen, linkageEnDe, debugPos, goalPos):  # E
    ...
    linkageLenREAL = np.sqrt(
        ((linkageLen[1]*linkageLen[1])+(linkageEnDe*linkageEnDe)))

    goalPos[0], goalPos[1] = limitCheck(
        goalPos, debugPos, [linkageLen[0], linkageLenREAL], 0.00001)

    if goalPos[0] < 0:
        goalPos[0] = - goalPos[0]
        mGenOut = linkageLenREAL*linkageLenREAL - \
            linkageLen[0]*linkageLen[0]-goalPos[0] * \
            goalPos[0]-goalPos[1]*goalPos[1]
        ...
        angleGenB = np.arcsin(
            (goalPos[1]-linkageLen[0]*np.cos(angleGenA))/linkageLenREAL)-angleGenA
        ...
        angleGenA = np.arccos((linkageLen[0]*linkageLen[0]+goalPos[1]*goalPos[1] -
                               linkageLenREAL*linkageLenREAL)/(2*linkageLen[0]*goalPos[1]))
        cGenOut = np.tan(angleGenA)*linkageLen[0]
        dGenOut = goalPos[1]-(linkageLen[0]/np.cos(angleGenA))
        angleGenB = np.arccos((cGenOut*cGenOut+linkageLenREAL *
                               linkageLenREAL-dGenOut*dGenOut)/(2*cGenOut*linkageLenREAL))
        ...
        nGenOut = (linkageLen[0]*linkageLen[0]+goalPos[0]*goalPos[0]+goalPos[1]
                   * goalPos[1]-linkageLenREAL*linkageLenREAL)/(2*linkageLen[0]*sqrtGenOut)
        ...
        mGenOut = (linkageLen[0]*linkageLen[0]+linkageLenREAL*linkageLenREAL-goalPos[0]
                   * goalPos[0]-goalPos[1]*goalPos[1])/(2*linkageLen[0]*linkageLenREAL)
...
class RaspArmS(threading.Thread):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            print('Import config [initPos]:\ninitPosA:%d\ninitPosB:%d\ninitPosC:%d\ninitPosD:%d' % (
                self.initPosA, self.initPosB, self.initPosC, self.initPosD))
            print('--- --- ---')
        except:
            self.initPosA = 300
            self.initPosB = 300
        ...
        try:
            self.servoNumA = int(
                self.configJson[self.servoPortIndex]['servoNumA'])
            self.servoNumB = int(
                self.configJson[self.servoPortIndex]['servoNumB'])
            self.servoNumC = int(
                self.configJson[self.servoPortIndex]['servoNumC'])
            self.servoNumD = int(
                self.configJson[self.servoPortIndex]['servoNumD'])

            print('Import config [servoPort]:\nservoNumA:%d\nservoNumB:%d\nservoNumC:%d\nservoNumD:%d' % (
                self.servoNumA, self.servoNumB, self.servoNumC, self.servoNumD))
            print('--- --- ---')
        except:
            self.servoNumA = 12
            self.servoNumB = 13
    ...
    def getPlanGoseList(self):
        return planGoseList

    def savePlanJson(self):
        content2write = json.dumps(planGoseList)  # jsonに書き込む
    ...
    def servoCtrl(self, initPos, servoNumInput, angleInput, directionInput):
        scOut = int(
            round(((ctrlRangeMax-ctrlRangeMin)/angleRange*angleInput), 0))
    ...
    def servoAngInput(self, angleInput):
        ...
        for i in range(0, 4):
            self.nowPos[i] = angleInput[i]

    def servoInitSet(self):
        print('Input your command here\n"w":pwm+\n"s":pwm-\n"q":servo+\n"a":servo-\n"x":set\n"c":cancel')
        servoList = [self.servoNumA, self.servoNumB,
                     self.servoNumC, self.servoNumD]

        posBuffer = [self.initPosA, self.initPosB,
                     self.initPosC, self.initPosD]
        servoNowSelected = 0
        while 1:
            command = input('"___"\nservoNum:%d is selected\nPWM:%d' % (
                servoList[servoNowSelected], posBuffer[servoNowSelected]))
            if command == 'w':
            ...
            self.pwm.set_pwm(servoList[servoNowSelected],
                             0, posBuffer[servoNowSelected])
    ...
    def moveXYZ(self, newXYZInput):
        ...
        for i in range(0, int(maxOffset)):
            ...
            if offsetX:
                genX = oldXYZInput[0] + \
                    ((newXYZInput[0]-oldXYZInput[0])*(i/maxOffset))
            genY = newXYZInput[1]
            if offsetY:
                genY = oldXYZInput[1] + \
                    ((newXYZInput[1]-oldXYZInput[1])*(i/maxOffset))
            genZ = newXYZInput[2]
            if offsetZ:
                genZ = oldXYZInput[2] + \
                    ((newXYZInput[2]-oldXYZInput[2])*(i/maxOffset))
    ...
    def gripper(self, command):
        ...
        looseNum = 50
        if command == 'catch':
            self.servoCtrl(self.initPosD, self.servoNumD, catchNum, 1)
            self.nowPos[3] = catchNum
        elif command == 'loose':
            self.servoCtrl(self.initPosD, self.servoNumD, looseNum, -1)
            self.nowPos[3] = looseNum
        else:
            try:
                self.servoCtrl(self.initPosD, self.servoNumD, command, 1)
    ...
    def xyzInput(self, xyzPosInput):
        ...
        allLengh = np.sqrt(
            xyzPosInput[0]*xyzPosInput[0] + xyzPosInput[1]*xyzPosInput[1])
        ...
        if xyzPosInput[0] > 0:
            ...
            genoutA = - \
                np.arctan(abs(xyzPosInput[1])/abs(xyzPosInput[0]))*180/np.pi+90
        else:
            genoutA = 0
        try:
            genoutBC = planeLinkageReverse([self.linkageLenB, self.linkageLenC], self.linkageLenD, [
                                           0, 0], [xyzPosInput[2], planeY])
            ...
            if self.modeNow == 'pen':
                ...
                self.servoCtrl(self.initPosD, self.servoNumD,
                               int(90 - genoutBC[0] - genoutBC[1]), 1)
            # print('done')
        except:
            pass
